...
def homomorphism_finder(seed_strand_set, knot_dict, wirt_num, transpositions):
    ...
    for gen_set in transpositions[wirt_num - 2]:
        ...
        for perm in perms_of_seed_strands:
            ...
            # If it does...
            if hmorph:
                # Return the Boolean variable indicating that it does and the seed-strand-to-generator mapping that
                # produced it
                return hmorph, sym_gen_set
    # If no hm was found across all permutations, hmorph will be false and we don't care what the program thinks the
    # sym_gen_set is
    return hmorph, sym_gen_set
...
def sym_group_crafter():
    ...
    transpositions = ((s3a,), (s4a, s4b, s4c), (s5a, s5b, s5c, s5d, s5e, s5f, s5g, s5h, s5i, s5j, s5k, s5l),
                      s6_set)  # the extra comma in the first entry forces (s3a) to be a one-element tuple
...
def generator_assign(wirt_num, perm, gen_set):
    ...
    for i in range(len(perm)):
        # Add an entry to the generating set dictionary with the letter of the seed strand from the knot dictionary
        # as the key and the Sn+1 generator as the entry
        sym_gen_set[perm[i]] = gen_set[i]
    # return the generating set, a dictionary with the keys being the seed strand letters and the enries being the
    # Sn+1 generators assigned to them
    return sym_gen_set
    ...
    while new_assignment:
        ...
        for overstrand in knot_dict:
            ...
            if overstrand in mapping:
                # Scan through all its understrand pairs (there may be more than one if the strand is the overstrand
                # of more than one crossing)
                for understrands in knot_dict[overstrand][1]:
                    ...
                    # If only the first understrand has been assigned...
                        ...
                        # A new assignment has been made, so another iteration is necessary to make sure all possible
                        # assigments have been made
                        new_assignment = True
                    # If only the second understrand has been assigned...
                        ...
                        # assigments have been made
                        new_assignment = True
                    # Otherwise, neither understrand has yet been assigned a transposition, so let another iteration
                    # happen
            # Otherwise, the overstrand has not been assigned a mapping yet, and there is nothing we can do yet,
            # so let another iteration happen
    # Return mapping, a dictionary with all strands that can be assigned a transpsition as keys, and their
    # corresponding two-tuple transpositions as entries.
    return mapping


# Function to test if the mapping given by the (aptly-named) mapping dictionary is a homoporphism from the
# fundamental group to the Sn+1 symmetric group
def homomorphism_tester(mapping, knot_dict):
    # If the length of mapping does not equal the length of the knot_dictionary, then something has gone badly wrong
    # and we don't even have a valid generator assignment for for each strand
    ...
    for overstrand in knot_dict:
        ...
        for understrands in knot_dict[overstrand][1]:
            # If ever one of the understrands' generators (it dosen't matter which understrand, since we're working
            # with two-cycles. Otherwise, it would) does not equal the Transpose product of the overstrand and other
            # understrand, the Wirtinger relation does not hold for this crossing, and this is not a homomorphism.
...
def transpose_product(overstrands, understrands):
    ...
    # If the two transpositions have one element in common...
        ...
        product = tuple(ostrand.symmetric_difference(
            ustrand))  # note that the product is converted back to a tuple so that it can be iterated over later
